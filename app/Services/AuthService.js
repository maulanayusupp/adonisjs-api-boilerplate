...
class AuthService {
  ...
  */
    ...
    try {
      ...
      if (user) {
        if (user.is_verified == 1) {
          ...
          const isDefaultPassword = await Hash.verify('lms2020', user.password);
          token.is_set_password = isDefaultPassword;

          return new Response({ data: token });
        } else {
          return new Response({ message: __('Please check your email, to activate your account', lang) }, 422);
        }
      }
  ...
  async autoLogin(body, auth) {
    ...
    const lang = body.input('lang') ? body.input('lang') : 'no';
    try {
  ...
  async getProfile(auth) {
    ...
    try {
      ...
      const newUser = await User.query().where('id',user.id)
        .with('profile')
        .first()
  ...
  async userRegister(body) {
    const lang = body.input('lang') ? body.input('lang') : 'no';
    const {name, email, password, mobile_phone, verification_method, username} = body.all();
    try {
      ...
      if (!user) {
        ...
        const profile = await Profile.query().where('user_id', user.id).withTrashed(true).first()
        profile.name = name;
  ...
  async sendSMSCode(body) {
    ...
    try {
      ...
      try {
        ...
        const from = Env.get('NEXMO_FROM');
        const to = user.mobile_phone;
        const text = __('Your LMS login, your code is', lang) + ' ' + code;
